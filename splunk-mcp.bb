...
;; Borrows session credentials from Microsoft Edge browser on macOS.
;; Original Author: Ag Ibragimov - github.com/agzam
;; Modified to use Microsoft Edge instead of Brave browser.
...
(defn get-keychain-password []
  (or (sh "security" "find-generic-password"
          "-s" "Microsoft Edge Safe Storage"
          "-a" "Microsoft Edge"
          "-w")
      (throw (ex-info "Could not retrieve Microsoft Edge keychain password" {}))))
...
(defn decrypt-cookie
  "Decrypt a Chromium cookie from Edge's SQLite DB.
  Copies DB to avoid lock contention with the browser."
  [cookie-name host-key aes-key-hex]
  (let [tmp-db  (doto (java.io.File/createTempFile "edge-" ".db") (.deleteOnExit))
        tmp-enc (doto (java.io.File/createTempFile "edge-" ".bin") (.deleteOnExit))
        tmp-dec (doto (java.io.File/createTempFile "edge-" ".bin") (.deleteOnExit))
        db-path  (.getAbsolutePath tmp-db)
        enc-path (.getAbsolutePath tmp-enc)
        dec-path (.getAbsolutePath tmp-dec)]
    (try
      (shell/sh "cp" edge-cookies-db db-path)
...
(defn discover-splunk-port
  ...
  []
  (let [tmp (doto (java.io.File/createTempFile "edge-" ".db") (.deleteOnExit))
        path (.getAbsolutePath tmp)]
    (try
      (shell/sh "cp" edge-cookies-db path)
  ...
  ([force?]
   (when (or force? (nil? @credentials))
     (let [port    (or (discover-splunk-port)
                       (throw (ex-info (str "No Splunk cookies in Edge for " splunk-host) {})))
